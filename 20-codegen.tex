...
我们编译器的测试用的模拟是 ravel，仓库位于 \url{https://github.com/Engineev/ravel}。
...
\begin{itemize}
  ...
  \item \texttt{.asciz} 表示一个 ASCII 字符串（字符串最后会自动加上结束符 \texttt{\textbackslash0})。
...
\texttt{sp} 的值即可，比如需要使用 \texttt{64 Bytes} 的栈，那么就在函数开头执行
...
函数指令的转换的过程较为繁琐，且思路非常简单清晰，本章中不会具体讲解如何转换。
如果对转换过程有疑惑的，可以通过 llc 辅助了解。
